...
class KeywordTransformer(ast.NodeTransformer):
    ...
    def visit_Return(self, node: ast.Return) -> typing.Union[ast.Return, ast.If]:
        ...
        return ast.If(
            test=ast.NameConstant(value=True, lineno=node.lineno, col_offset=node.col_offset),  # if True; aka unconditional, will be optimized out
            body=[
                # yield the value to be returned
                ast.Expr(value=ast.Yield(value=node.value, lineno=node.lineno, col_offset=node.col_offset), lineno=node.lineno, col_offset=node.col_offset),
                # return valuelessly
                ...
                ast.If(
                    ...
                    body=[
                        ast.Expr(
                            ...
                            value=ast.Call(
                                # globals().pop
                                func=ast.Attribute(value=self.globals_call(node), attr="pop", ctx=ast.Load(), lineno=node.lineno, col_offset=node.col_offset),
                                args=[
                                    # 'x'
                                    ast.Str(s=target.id, lineno=node.lineno, col_offset=node.col_offset)
                                ],
                                keywords=[],
                                lineno=node.lineno,
                                col_offset=node.col_offset,
