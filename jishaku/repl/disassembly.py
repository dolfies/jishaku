...
def disassemble(
    code: str, scope: typing.Optional[Scope] = None, arg_dict: typing.Optional[typing.Dict[str, typing.Any]] = None
    ...
    for instruction in dis._get_instructions_bytes(  # type: ignore
        co.co_code, co.co_varnames, co.co_names, co.co_consts, co.co_cellvars + co.co_freevars, dict(dis.findlinestarts(co)), line_offset=0
    ):
...
def format_ast_block(
    node: typing.Union[typing.List[ast.AST], ast.AST], header: str = "", level: int = 0, through: bool = False, use_ansi: bool = True
) -> typing.Generator[str, None, None]:
    """
    ...
    for index, item in enumerate(node):
        ...
        branch, stalk = (maybe_ansi(f"{branch} {header}", level, use_ansi), maybe_ansi(stalk, level, use_ansi))
...
def format_ast_node(node: ast.AST, level: int = 0, use_ansi: bool = True) -> typing.Generator[str, None, None]:
    ...
    for index, field in enumerate(fields):
        yield from format_ast_block(getattr(node, field), header=field, through=index < len(fields) - 1, level=level, use_ansi=use_ansi)
