...
class VoiceFeature(Feature):
    ...
    async def connected_check(ctx: ContextA):
        ...
        if (
            ...
            or (not ctx.guild.voice_client.is_connected() if isinstance(ctx.guild.voice_client, discord.VoiceClient) else False)
        ):
    ...
    async def jsk_vc_join(self, ctx: ContextA, *, destination: typing.Union[discord.VoiceChannel, discord.Member] = None):  # type: ignore
        """
